...
tmux_theme_status_enable=$(get_tmux_option -g "$opt_tmux_theme_status_enable")
declare -r tmux_theme_status_enable

# exit if status script is disabled
if [ "$tmux_theme_status_enable" != "true" ]; then
  exit 0
fi
